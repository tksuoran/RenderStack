...
bool application::initialize_services()
{
    ...
    if (gui_renderer)
    {
        gui_renderer->connect(renderer, shader_monitor_);
    }

    if (programs_)
    {
        programs_->connect(renderer, shader_monitor_);
    }

    if (textures_)
    {
        textures_->connect(renderer);
    }

    if (debug_renderer_)
    {
        debug_renderer_->connect(renderer, gui_renderer, programs_, application_);
    }

    if (quad_renderer_)
    {
        quad_renderer_->connect(renderer);
    }

    if (forward_renderer_)
    {
        forward_renderer_->connect(renderer, debug_renderer_, programs_);
    }

    if (deferred_renderer_)
    {
        deferred_renderer_->connect(renderer, programs_, quad_renderer_, light_mesh_);
    }

    if (l_d_renderer_)
    {
        l_d_renderer_->connect(renderer, programs_, light_mesh_);
    }

    if (id_renderer_)
    {
        id_renderer_->connect(renderer, programs_);
    }

    if (light_mesh_)
    {
        light_mesh_->connect(renderer, programs_);
    }

    if (scene_manager_)
    {
        scene_manager_->connect(programs_, renderer);
    }

    if (game_)
    {
        game_->connect(renderer,
                       shader_monitor_,
                       gui_renderer,
                       programs_,
                       textures_,
