...
model::model()
    : m_name(""), m_frame(nullptr), m_geometry_mesh(nullptr), m_material(nullptr)
{
    m_frame = make_shared<renderstack::scene::frame>();
}

model::model(string const &name)
    : m_name(name), m_frame(nullptr), m_geometry_mesh(nullptr), m_material(nullptr)
{
    m_frame = make_shared<renderstack::scene::frame>();
}
...
model::model(
    ...
    : m_name(name), m_frame(nullptr), m_geometry_mesh(geometry_mesh), m_material(material_)
{
