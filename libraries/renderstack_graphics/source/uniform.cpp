...
uniform::uniform(string const &name, std::shared_ptr<uniform_block> block, size_t count, gl::active_uniform_type::value type)
    : m_name(name), m_count(count), m_type(type), m_precision(precision::highp), m_is_array(false), m_texture_unit_index(0), m_sampler(nullptr), m_location(-2), m_block_index(-1), m_block(block), m_index_in_block(block->num_uniforms()), m_offset_in_block(block->offset())
{
}

uniform::uniform(string const &name, int location, size_t count, gl::active_uniform_type::value type)
    : m_name(name), m_count(count), m_type(type), m_precision(precision::highp), m_is_array(false), m_texture_unit_index(0), m_sampler(nullptr), m_location(location), m_block_index(-1), m_index_in_block(0), m_offset_in_block(0)
{
}
