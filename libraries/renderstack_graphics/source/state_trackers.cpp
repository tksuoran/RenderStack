...
state::state()
    : vertex_array_binding(nullptr), current_program(nullptr), transform_feedback_binding(0), active_texture(0), draw_framebuffer_binding(0), read_framebuffer_binding(0), renderbuffer_binding(0), current_query(0)
{
...
state::state(state const &other)
    : vertex_array_binding(other.vertex_array_binding), current_program(other.current_program), transform_feedback_binding(other.transform_feedback_binding), active_texture(other.active_texture), draw_framebuffer_binding(other.draw_framebuffer_binding), read_framebuffer_binding(other.read_framebuffer_binding), renderbuffer_binding(other.renderbuffer_binding), current_query(other.current_query)
{
    for (int i = 0; i < RS_TEXTURE_UNIT_COUNT; ++i)
