...
button::button(
    ...
    : area(renderer, style), m_text_buffer(renderer, style->font()), m_ninepatch(renderer, style->ninepatch_style()), m_dirty(true), m_trigger(false)
{
    set_name(label);
    set_label(label);
}
...
glm::mat4 &button::text_frame()
{
    return m_text_frame;
}
glm::mat4 &button::background_frame()
{
    return m_background_frame;
}
class ninepatch &button::ninepatch()
{
    return m_ninepatch;
}
...
class text_buffer &button::text_buffer()
{
