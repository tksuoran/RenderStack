...
void font::render(renderstack::graphics::renderer &renderer)
{
   ...
   try
   {
      ...
      log() << "face flags    ";
      if (face->face_flags & FT_FACE_FLAG_SCALABLE        ) log() << "scalable ";
      if (face->face_flags & FT_FACE_FLAG_FIXED_WIDTH     ) log() << "fixed_width ";
      if (face->face_flags & FT_FACE_FLAG_SFNT            ) log() << "sfnt ";
      if (face->face_flags & FT_FACE_FLAG_HORIZONTAL      ) log() << "horizontal ";
      if (face->face_flags & FT_FACE_FLAG_VERTICAL        ) log() << "vertical ";
      if (face->face_flags & FT_FACE_FLAG_KERNING         ) log() << "kerning ";
      if (face->face_flags & FT_FACE_FLAG_FAST_GLYPHS     ) log() << "fast_glyphs ";
      if (face->face_flags & FT_FACE_FLAG_MULTIPLE_MASTERS) log() << "multiple_masters ";
      if (face->face_flags & FT_FACE_FLAG_GLYPH_NAMES     ) log() << "glyph_names ";
      if (face->face_flags & FT_FACE_FLAG_EXTERNAL_STREAM ) log() << "external_stream ";
      if (face->face_flags & FT_FACE_FLAG_HINTER          ) log() << "hinter ";
      if (face->face_flags & FT_FACE_FLAG_CID_KEYED       ) log() << "cid_keyed ";
      if (face->face_flags & FT_FACE_FLAG_TRICKY          ) log() << "tricky ";
      log() << '\n';
      log().flush();
      log() << "style flags   ";
      if (->style_flags & FT_STYLE_FLAG_ITALIC) log() << "italic ";
      if (->style_flags & FT_STYLE_FLAG_BOLD) log() << "bold ";
      log() << '\n';
      log().flush();
      ...
      for (auto i = m_chars.cbegin(); i != m_chars.cend(); ++i)
      {
         ...
         try
         {
            ...
            if (g->bm_width()  > max_bm_size) max_bm_size = g->bm_width();
            if (g->bm_height() > max_bm_size) max_bm_size = g->bm_height();
            ...
            if (m_outline_thickness > 0.0f)
            {
               ...
               if (og->bm_width()  > max_bm_size) max_bm_size = og->bm_width();
               if (og->bm_height() > max_bm_size) max_bm_size = og->bm_height();
            }
         }
         catch (...)
         {
