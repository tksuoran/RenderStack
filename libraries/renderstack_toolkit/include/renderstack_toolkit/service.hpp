...
public:
   ...
   void register_as_service() { m_is_registered = true; }
   void unregister() { m_is_registered = false; }
   void initialize();
   bool ready() const;
   void remove_dependencies(std::set<std::shared_ptr<service> > const &remove_set);
   std::set<std::shared_ptr<service> > const &dependencies() { return m_dependencies; }
